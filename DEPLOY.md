...
7. «передай @другой_пользователь_из_ALLOWED_USER_IDS, что...» → черновик →
   «Отправить» → сообщение приходит получателю с пометкой «через Арину»
   (нужен второй реальный аккаунт из `ALLOWED_USER_IDS` — без него можно
   проверить только отказ «не нашла пользователя»)
8. Email-напоминание — см. раздел 7 ниже (нужен `RESEND_API_KEY`)
9. `/delete_my_data` → все данные удалены (включая транзакции и лог писем)
